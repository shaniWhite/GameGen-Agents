...
import openai
import shutil
import automate_game
import logging
from agents.code_repair import Code_Repair_Agent
from agents.code_updater import GameUpdater_Agent
from tools.file_tools import parse_file_structure
from tools.run_game import run_game, close_game
from agents.planners import plan_project
from agents.developers import developer_agent
...
async def main():
    ...
    game_name, window_size, file_structure = parse_file_structure(final_plan)
    ...
    for file_name, file_description in file_structure:
        task = asyncio.create_task(developer_agent(file_name, file_description, final_plan))
        tasks.append(task)
    ...
    while True:
        ...
        if error_message is None:
            ...
            error_details = automate_game.run_game(game_name)
